...
namespace jcd {
	...
	do_sort(BidirectionalIterator first, BidirectionalIterator last) {
		...
		auto left_chunk = std::make_shared<chunk_to_sort_t>(chunk_to_sort_t{ first, partition_ptr });
		auto left_task = left_chunk->promise.get_future();
		chunks_.push(left_chunk);
